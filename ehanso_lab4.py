...
def bruteforce(G):
	...
	for subset in P: 
		...
		for v in G.V:
			for e in G.E[v]:
				if (v not in subset and e not in subset):
					works = False 
				else:
					pass
		if works == True:
			if best_set is None or len(best_set) > len(subset):
				best_set = subset
	return best_set
	...
	while G.V:
		...
		for x in G.V:
			...
			if edges > max_edges:
				...
				max_vertex = x
		C.add(max_vertex)
		del_edges = G.E[max_vertex]
		if len(del_edges) != 0:
			while del_edges:
				e = next(iter(del_edges))
				G.remove_edge(max_vertex, e)
		G.remove_vertex(max_vertex)
		if G.count_edges() == 0:
			return C
...
def matching(G):
	...
	while G.count_edges() > 0:
		...
		del_edges = G.E[first_vertex]
		while del_edges:
			e = next(iter(del_edges))
			G.remove_edge(first_vertex, e)
			
		del_edges = G.E[second_vertex]
		while del_edges:
			e = next(iter(del_edges))
			G.remove_edge(second_vertex, e)
			
		G.remove_vertex(first_vertex)
		G.remove_vertex(second_vertex)
		C.add(first_vertex)
		C.add(second_vertex)
